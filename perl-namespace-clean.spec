%{?scl:%scl_package perl-namespace-clean}
...
Name:		%{?scl_prefix}perl-namespace-clean
...
BuildRequires:	%{?scl_prefix}perl-interpreter >= 4:5.12
...
BuildRequires:	%{?scl_prefix}perl(Package::Stash) >= 0.23
BuildRequires:	%{?scl_prefix}perl(strict)
BuildRequires:	%{?scl_prefix}perl(vars)
BuildRequires:	%{?scl_prefix}perl(warnings)
# Test Suite
BuildRequires:	%{?scl_prefix}perl(Config)
BuildRequires:	%{?scl_prefix}perl(DB)
...
BuildRequires:	%{?scl_prefix}perl(sort)
...
Requires:	%{?scl_prefix}perl(B::Hooks::EndOfScope) >= 0.12
Requires:	%{?scl_prefix}perl(Package::Stash) >= 0.23

# Avoid unwanted requires/provides that come with the test suite
%{?perl_default_filter}
# namespace::clean::_Util is a private package
%global __requires_exclude %{?__requires_exclude:__requires_exclude|}^%{?scl_prefix}perl\\(namespace::clean::_Util\\)
...
%changelog
* Mon Jan 06 2020 Jitka Plesnikova <jplesnik@redhat.com> - 0.27-13
- SCL
